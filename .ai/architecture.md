...
| `runtime/objects/` | 各具体对象类型：`Int`（裹 `BigInt`）、`Decimal`（裹 `BigDec`）、`Str`（`u32string`，按码点）、`Tuple`（`vector<ObjectRef>`）、`Exception`（`BaseException` 及其全部子类共用，只存 `.args` 元组）、`singletons.h`（`NamedSingleton` 无负载单例 + `Bool`）。 |
