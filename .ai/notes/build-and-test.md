...
验证，不要让用户代跑。构建失败会跳过测试、返回非零；输出里会分别报 `SL_Cpp_Numeric_Tests`、
`SL_Cpp_Lexer_Tests`、`SL_Cpp_Parser_Tests`、`SL_Cpp_Analyzer_Tests` 四个独立可执行文件各自的
用例数/断言数。如果构建失败但报错信息在 Bash 输出里
