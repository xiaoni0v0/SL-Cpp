...
json AstNodeLiteralTuple::to_json_impl(const bool include_pos) const {
    ...
    if (include_pos)
        return json{
            {"type", "LiteralTuple"}, {"pos", pos_to_json(pos_)}, {"items", std::move(items)}
        };
...
json AstNodeLiteralList::to_json_impl(const bool include_pos) const {
    ...
    if (include_pos)
        return json{
            {"type", "LiteralList"}, {"pos", pos_to_json(pos_)}, {"items", std::move(items)}
        };
...
json AstNodeLiteralDict::to_json_impl(const bool include_pos) const {
    ...
    if (include_pos)
        return json{
            {"type", "LiteralDict"}, {"pos", pos_to_json(pos_)}, {"items", std::move(items)}
        };
