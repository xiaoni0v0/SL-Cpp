...
void SyntaxChecker::check(const AstNodeCall *node) {
    ...
    ctx_.star_ok = false;
    ctx_.double_star_ok = false;
    check(node->object_.get());

    // ** 之后不能再出现其他参数
    bool seen_double_star = false;
    ctx_.star_ok = true;
    ctx_.double_star_ok = true;
    ...
    ctx_.star_ok = false;
    ctx_.double_star_ok = false;
    for (const auto &[key, val] : node->kwargs_) check(val.get());
...
void SyntaxChecker::check(const AstNodeIndex *node) {
    ...
    ctx_.double_star_ok = false;
    check(node->object_.get());
    for (const auto &a : node->args_) check(a.get());
    ctx_ = saved;
}
...
void SyntaxChecker::check_lvalue(const AstNode *node, const bool allow_star) const {
    ...
    if (const auto *s = dynamic_cast<const AstNodeStar *>(node)) {
        if (!allow_star) error("* not allowed in assignment target", s->row_, s->col_);
        check_simple_lvalue(s->operand_.get());
        return;
    }
    if (const auto *t = dynamic_cast<const AstNodeLiteralTuple *>(node)) {
        check_unpack_items(t->items_);
        return;
    }
    if (const auto *l = dynamic_cast<const AstNodeLiteralList *>(node)) {
        check_unpack_items(l->items_);
        return;
    }
    error("invalid assignment target", node->row_, node->col_);
}
...
void SyntaxChecker::check_unpack_items(const std::vector<AstNodePtr> &items) const {
    bool has_star = false;
    for (const auto &item : items) {
        if (dynamic_cast<const AstNodeStar *>(item.get())) {
            if (has_star) error("multiple * in unpacking target", item->row_, item->col_);
            has_star = true;
        }
        check_lvalue(item.get(), true);
    }
}
