...
调用 `x(arg, kwarg=v, ...)` 时，**先检查 `x` 是否可调用**：若 `type(x)` 不是内置类 `callable` 的子类，直接抛 `TypeError`
、不进行调用。可调用则按运算符分派（见 3.8）执行 `type(x).__op_call__(x, ...)`——`callable` 把 `__op_call__` 声明为抽象，子类必须实现。

以下规则描述**函数对象**被调用时（即 `function.__op_call__`）的实参与形参对应；类对象的调用见 3.4.8（构造实例），其他可调用对象由各自的
`__op_call__` 决定。
...
1. type(o) 的 MRO 上有 attr，且 isinstance(类属性, Descriptor) → 返回 它.get(o, type(o))
