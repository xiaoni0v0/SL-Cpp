...
比较运算符的额外回退：`<`、`<=`、`>`、`>=`、`==`、`!=` 这六个，每一侧的方法落空（没有该方法，
或调用返回 `NotImplemented`）之后，都再试一次该侧的 `__op_cmp__` / `__rop_cmp__`，然后才换边 / 报错。
...
`!=` 另有一条回退：`__op_ne__` 落空后先试 `__op_eq__`、取其真值的逻辑非（`not`），再试 `__op_cmp__`，反向一侧同理。
这种互推只发生在 `==` 与 `!=` 之间，`<`、`<=`、`>`、`>=` 之间没有。
...
要放多条请自己用复合表达式包起来（`eval('{ a; b }')`）。

`code` 顶层不允许出现 `;`，多余的结尾分号也不行：`;` 是硬终止符（2.2.1），写出来就是在断言“这里是
一条表达式的边界”，而这个入口只收一条，任何顶层的 `;` 都是多余的。前后的空白与换行不受此限
（换行是软终止，前后没有内容时它什么也没分隔）。复合表达式内部的 `;` 不算顶层，不受影响。

`eval` 是内联语义，`eval('code')` 等价于直接把整个调用用 `code` 替换掉。

解析、检查 `code` 时，它的静态外层环境是空的，既不在任何 Program 内，也不在任何循环内。
...
- 有 `Infinity`、`-Infinity`、`NaN`、`sNaN`、负零。
  ...
  这类构造显式产生。负零不同，它不由任何信号产生，运算照常给得出来（`0 * -1`、`0 / -1` 都是 `-0`）；
