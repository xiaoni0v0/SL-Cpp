...
| 55       | `in`                                                                             | 成员测试           | L      | 2        | 不像比较运算符、`is` 那样支持链式：`a in b in c` 就按普通左结合解析成 `(a in b) in c`（`in` 的结果通常是 bool，再 `in` 一次多半在运行期抛 `TypeError`）。语义见 3.4.2                                                                                                                                                                                                                                                                                                                                                  |
...
- `x in y`，成员测试，返回 `x` 是否是 `y` 的成员之一；由 `y`（**右**操作数）定义，见 3.8；
  内置容器一律返回 bool；自定义类型返回其 `__op_contains__` 的返回值原样，SL 不强制转成 bool；
...
- 若发生异常，从上到下检查每个 `except` 的 `Exception`（可能不止一个，每个 `except` 内也可能有多个 `Exception`），
  对其求值并以 `isinstance(异常对象, Exception)` 判定是否匹配；一旦匹配，对 `expr2` 求值，其值即整个表达式的值，不再检查后面的 `except`；
...
3. 在 `expr2` 求值结束、退出该 `except` 子句时，恢复 `__except__`：
    - 若进入前已存在，则恢复为其原值；
...
`in` 是唯一的例外，**不走上面那套两步分派**：`a in b` 只在 `b`（右操作数）的类上查找
`__op_contains__` 并调用 `type(b).__op_contains__(b, a)`，没有反向方法、没有回退，找不到就抛
`TypeError`。方法名带 `contains` 而不是 `in`，正是为了让"谁实现它"从名字上就看得出来——
它读作"`b` 含不含 `a`"。
