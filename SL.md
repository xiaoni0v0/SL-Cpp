...
查不出来的（例如来自 `eval` 现场编译出的代码，见 4.1.14）由这层运行时拦截兜底，同样抛出 `SyntaxError`。
...
运行期维护一个帧栈。
每次函数调用、类体执行、执行一份文件（包括当前脚本本身、`import`、`eval_isolated`），都会压入一个帧，栈底为全局帧。
...
但文件级别的 Program（不管是 `import` 导入的模块，还是 `eval_isolated` 临时构造出的那种，见 3.4.5/4.1.15）弹出之后不会死亡，
局部字典（也就是这份文件自己的 `_G`）会一直保留、继续存在，不区分是不是被缓存。
...
#### 4.1.15 `eval_isolated(code, globals=None)`

`code` 为 `str`，在调用这一刻按 2.2.1 的规则解析为若干条表达式，构成一个全新的、独立的文件，
与调用处的文件、局部/全局变量、global 标记集完全无关，不共享任何状态。

`eval_isolated(code)` 语义上等价于：
把 `code` 单独存成一个文件，`globals` 若传入则把它并入这个文件的 `_G`，再把这个文件当作模块导入；
`eval_isolated` 的返回值就是这个模块对象的 `__return__` 属性。

但不同于 `import`：这个模块对象不进解释器的模块缓存，它的帧依然遵循"文件级 Program 弹出不死亡"
这条规则（3.10.1），只是没有缓存强引用着它——调用结束后，只要没有任何东西还引用它，就会被当成
普通对象回收；如果 `code` 里定义的函数逃逸出来了，那就还有东西引用着它，帧照常存活、可用，引用
捕获照常有效，不会因为没被缓存就死亡或失效。
