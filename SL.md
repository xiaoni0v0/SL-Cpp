...
| 30   | `and`                                                                            | 逻辑与       | L   | 2    |                                                                                                                                                                                                                                                                             |
| 20   | `or`                                                                             | 逻辑或       | L   | 2    |                                                                                                                                                                                                                                                                             |
| 10   | `=`, `+=`, `-=`, `*=`, `**=`, `/=`, `//=`, `%=`, `&=`, `\|=`, `^=`, `<<=`, `>>=` | 赋值、复合赋值   | R   | 2    | `=` 左边的**左值**（可以出现在赋值左边的表达式）包括：<ul><li>标识符；</li><li>`target.attr`（属性访问，不管 `target` 本身是什么结构）；</li><li>`target[index, ...]`（元素访问，同上）；</li><li>由以上三种左值组成的元组/列表字面量 `(lv1, lv2, ...)`/`[lv1, lv2, ...]`（**解构赋值**），<br>其中至多一个左值可以带 `*` 前缀（收集剩余元素），可嵌套，语义同 Python 的打包解包。</li></ul> |
...
**注**：`*`/`**` 展开（3.6）与装饰器 `@`（2.2.8）不是运算符，不可重载，故不列入上表；它们与运算符的结合方式见各自章节。
...
`[ALL_CAPTURE]` 为**捕获列表**，语法、语义与 2.2.6 所述完全一致（值捕获、引用捕获均可用），
只是建立时机从函数建立时换成类体建立时，具体处理时机见 3.4.7。
...
- `@` 可以连用：`@d1 @d2 expr` 即 `d1(d2(expr))`，值与求值顺序见 3.4.8。
...
- Program 的值：从前到后依次对其中各表达式求值；一旦某个表达式的求值触发了 `return`（见 3.4.5.6），
...
- `x / y`, `x // y`, `x % y`
  ...
  对于 `x` 是 str，`%` 表示字符串格式化（同 Python 旧式 `%` 格式化的一个子集），`y` 为替换参数：
  `y` 是 tuple 则按顺序逐个替换，否则 `y` 整体当成唯一一个替换参数；
  格式串里 `%` 后跟一个可选的 `0`（零填充，默认空格填充）、一个可选的十进制宽度、一个可选的
  `.精度`（仅 `%f` 有意义，默认 6 位小数），最后是转换字符之一：
  `%s`（`str(y)`）、`%d`（要求 `y` 是 int）、`%f`（要求 `y` 是数字）、`%%`（字面量 `%`）；
...
- `x & y`, `x | y`
    - 对于 int，分别返回按位与、按位或；
    - 对于 set，返回它们的交集、并集；
    - 对于 `|`，`x`、`y` 都是 dict 时，返回两者合并后的新 dict（同 Python 3.9+ 的 `dict` 合并）：
      重复的 key 以 `y` 中的值为准，但该 key 在结果中的位置保持 `x` 里原来的位置；
      `y` 中不重复的 key 按 `y` 中的顺序追加在结果末尾；
...
4. 在该帧中从前到后对类体的各表达式求值；类体中可以使用 `return` 提前中止这一步（3.4.5.6），
   其值被丢弃，不影响下一步；
...
**注**：以上判定按表达式本身而非书写形式。`(expr)` 与 `expr` 是同一个表达式，
故 `{(**d)}` 等价于 `{**d}`、`f((*a))` 等价于 `f(*a)`；而 `**d + x` 里的展开是加法的操作数，不在合法位置上。
...
但文件级别的 Program（不管是 `import` 导入的模块，还是 `eval_isolated` 临时构造出的那种，见 4.1.14/4.1.16）弹出之后不会死亡，
...
#### 4.1.14 `import(module_name, lazy: bool = False, force: bool = False)`

导入名称为 `module_name` 的模块，返回对应的模块对象。

按 `module_name` 找到一份 SL 源文件。
若找不到，则在内置模块名里找（目前仅有 `math`、`time`、`numbers`、`protocols`），内置实现。
若找到了 SL 源文件：

1. 新建一个模块对象，新建一个帧，压入帧栈；
2. 把这份源文件的内容当一个 Program 在这个新帧上执行；
3. 执行完毕（或被 `return` 提前结束）后弹出该帧。但这个帧不会死亡，它的 `_G` 会一直保留，直接作为 `attrs(模块对象)`；
4. 这个 Program 的值记入模块对象的 `__return__` 属性。

`lazy` 参数表示是否延迟加载模块，若为 `True`，则在首次获取其属性时才会加载模块（即把上面 1-4 步
延后到那一刻才做）。
**注意**：延迟加载可能导致异常的延迟发生。

模块对象一旦建立，就会被解释器缓存，在解释器停止之前永远存在、永远不被回收；
对同一个 `module_name` 重复调用 `import`，返回的是同一个模块对象，不会重新执行文件。

`force` 为 `True` 时，丢开缓存的旧模块（但原模块对象仍然可用，只是不再缓存），重新完整走一遍上面 1-4 步，建立一个全新的模块对象。
`force` 只影响要不要复用旧缓存，可以和 `lazy` 同时使用，`lazy` 依然决定这次（新的）导入是立即执行还是延迟到首次取属性时才执行。

**易错提醒**：在模块源文件里定义一个名为 `__op_call__` 的东西，不会让这个模块对象本身变得可调用。
调用 `x(...)` 查的是 `type(x)` 的 `__op_call__`（3.8）。
模块对象的类型是 Module 这个内置类，不是这份源文件；文件里的 `__op_call__` 只是模块的一个普通属性。
...
`TypeVar(bound=None)`。用作类型注解，见 3.4.6。
