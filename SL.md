...
语法：`del target`。`target` 的**根节点**必须是标识符或属性访问 `expr.attr` 之一（其内部子表达式不限），即只有两种形式：

1. `del identifier` —— 删除变量（语义见 3.10.3）；
2. `del expr.attr` —— 删除属性（语义见 3.9.3）。
...
del a              # 标识符
del a.x            # 根为属性访问
del a['key'][0].x  # 根为属性访问，内部任意
```

不合法（根为索引，抛 `SyntaxError`）：
...
1. （计数-步进模式）
   ...
   其中 `init`, `cond`, `inc` 为表达式或空，三者用 `;` 或至少一个换行无歧义地分割。若某个槽为空，则必须使用 `;`。
...
3. （计数-迭代模式）
   ...
   其中 `identifier` 为标识符，`iterable` 为表达式。

以上三种均可在 `for` 后加 `$` 来从计数模式变为收集模式，例如 `for $ (cond) expr`。

**注意**：SL 中没有 `while` 关键字。原因是 `while` 的功能可完全被 `for` 替代，属于冗余。
...
当异常匹配某个 `except`，则将对其 `expr2` 求值前，会将被捕获的异常对象绑定到当前作用域的变量 `__except__`：

1. 若当前作用域**已存在** `__except__`，先将其原值临时保存（被 shadow）；
2. 将被捕获的异常对象赋给 `__except__`；
3. 在 `expr2` 求值结束、退出该 `except` 块时（无论正常结束还是因 `expr2` 内再次抛出异常而离开），恢复 `__except__`：
    - 若进入前已存在，则恢复为其原值；
...
1. `*expr` 只能出现在元组字面量、列表字面量、函数调用、索引中；
...
求属性运算符 `obj.attr` 不可重载，固定按下述算法（SL 没有 `__getattr__`/`__setattr__`/`__delattr__` 这类拥有者侧魔术方法，属性行为只由值侧的描述器决定）。设 `obj` 的类为 `C`。

每次属性操作先做**描述器解析**：沿 `C` 及其基类查找名为 `attr` 的属性，若找到的值 `d` 的类提供 `__get__` 方法，则称 `d` 为该属性的**描述器**。描述器只在类上识别，存于实例自身字典的同名值不算描述器。

唯一原则：**有描述器就全交给它，它没实现的操作即禁止；没有描述器则在实例自身字典上自由读、写、删。** 描述器恒优先于实例字典（读取也是），故类一旦为某属性设了描述器，实例字典无法遮蔽它（因此方法不能按实例覆盖）。

#### 3.9.1 读 `obj.attr`

1. 有描述器 `d` → 返回 `d.__get__(obj)`；
2. 否则实例自身字典有 `attr` → 返回该值；
3. 否则 `C`（及基类）上有非描述器的同名属性 → 返回该值；
4. 否则 → `AttributeError`。

#### 3.9.2 写 `obj.attr = v`

1. 有描述器 `d`：`d` 有 `__set__` → 调 `d.__set__(obj, v)`；否则 `AttributeError`；
2. 否则 → 在实例自身字典中写入 `attr`（无则新建）。

#### 3.9.3 删 `del obj.attr`

1. 有描述器 `d`：`d` 有 `__delete__` → 调 `d.__delete__(obj)`；否则 `AttributeError`；
2. 否则 → 删除实例自身字典中的 `attr`（无则 `AttributeError`）。

以上以实例访问为例。经由类访问 `C.attr` 同理，但命中描述器时以 `d.__get__(None)` 调用（故 `C.f` 得到原函数）。

#### 3.9.4 常见描述器

均为包装一个函数的描述器对象：

- **方法**：函数本身即描述器，`__get__(obj)` 返回把 `obj` 预填为第一个实参 `self` 的绑定方法；`obj` 为 `None`（经类访问）时返回原函数。
- **静态方法**：`__get__` 原样返回函数，不绑定。
- **类方法**：`__get__` 返回把 `C` 预填为第一个实参的绑定方法。
- **只读 / 计算属性**：`__get__` 计算返回值；给了 `__set__` 即可写、未给则只读。

属性能否读/写/删，全由类作者放不放、放哪种描述器决定；不放则三者皆自由。

#### 3.9.5 `__dict__`

实例的属性存于其自身的**属性字典**，由只读描述器 `__dict__` 暴露。它与作用域里的 `_G`/`_L`（见 3.10.1）同理——是一个**实时视图**：可读、可改其内容，但不能重绑或删除视图本身：

- 读 `obj.__dict__` → 返回那本**活的**字典；
- `obj.__dict__ = ...`、`del obj.__dict__` → 该描述器无 `__set__`/`__delete__` → `AttributeError`；
- 但对它返回的字典本身可增改键（`obj.__dict__['k'] = v`）、用方法删键（`obj.__dict__.pop('k')`；`del` 不接受下标）——这是直接操作实例属性的入口。

注意：即便用 `obj.__dict__` 塞入与某描述器同名的项，因描述器恒优先，`obj.attr` 仍走描述器、该项被忽略——后门绕不过描述器。
...
v.get            # 方法 → 绑定方法（self = v）
v.get()          # 5
v.x = 9          # x 无描述器 → 写入 v 的属性字典
v.__dict__       # 活字典 {'x': 9}
del v.x          # x 无描述器 → 从属性字典删除（根为 Attr，合法）
v.__dict__ = {}  # 重绑 __dict__ → 无 __set__ → AttributeError
```
...
`if`、`for`、`try`、`{ }` 不引入作用域；函数可嵌套调用、类可嵌套定义，故运行期可同时存在多个局部作用域。

#### 3.10.1 帧栈

运行期维护一个帧栈，每次**函数调用**或**类体执行**压入一个帧、结束时弹出，栈底为全局帧（文件作用域）。
...
f()()                       # 再次调用 g 时 f 已返回，链为 g→全局 → 2
```

*注*：SL 不支持闭包。若只需在函数对象建立时以快照方式捕获一个值，可用有默认值的形参：
