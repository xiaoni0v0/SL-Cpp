...
`if`、`for`、`try`、`{ }` 不引入作用域；函数可嵌套调用，故运行期可同时存在多个局部作用域。

#### 3.9.1 帧栈

运行期维护一个帧栈，栈底为全局帧 `_G`，每次函数调用压入一个局部帧、返回时弹出，栈顶为当前帧 `_L`；
每个帧维护一个变量名到对象的字典，读/写/删一个变量即操作某帧中的对应项；
全局作用域中 `_L` 即 `_G`。

确定“标识符 `identifier` 操作哪个帧”这个过程称为**作用域确定**，规则见 3.9.3 所述。

#### 3.9.2 `global` 声明

对 `global identifier` 求值，会进行：

1. 若 `identifier` 已经在 global 标记集中，则跳过后续步骤；
2. 若 `_L` 中已有该项，则将其删除，再把标识符加入标记集。
...
2. 否则：
    - 读取：从当前帧 `_L` 起，沿帧栈逐帧向栈底 `_G` 查找，返回首个含该项之帧中的值；
...
*注*：SL 不支持闭包。若只需在函数对象建立时以快照方式捕获一个值，可用参数默认值：
