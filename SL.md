...
|------|----------------------------------------------------------------------------------|-----------|-----|------|
| 170  | `x[arg, ...]`, `x(arg, kwarg=v, ...)`, `x.attribute`                             | 索引、调用、属性  | L   | -    |
| 160  | `x?`, `x!`                                                                       | 问号、感叹号    | L   | 1    |
| 150  | `**`                                                                             | 幂运算       | R   | 2    |
...
**注**：`cond` 不能是裸的普通赋值 `=`（避免 `if (x = y)` 这种大概率是 `==` 手误的写法）；
...
**注**：步进模式中的 `cond` 不能是裸的普通赋值 `=`（原因同 2.2.5.1），需显式再套一层括号才能这样写。
...
4. 对运算符：
    ...
    6. 对赋值运算符（左值定义见 2.1.5）
        ...
        4. 复合赋值 `target op= expr`（`target` 为标识符、属性访问或元素访问）
...
`code` 为 `str`，在调用这一刻按 2.2.1 的规则解析为若干条表达式。

`eval(code)` 等价于把这次调用原地替换成复合表达式 `{code}`。

`{}` 不引入新作用域（3.10），所以 `_L`/`_G`（含 global 标记集）、`code` 中定义的函数的捕获定义帧、
`return`/`break`/`continue` 的合法性与目标，全部沿用调用处当前帧。

返回值为 `{code}` 的值，即最后一条表达式的值，空 `code` 的值为 `None`。
若中途被 `return` 提前带出（使外层函数返回），则返回 `None`。

#### 4.1.16 `eval_isolated(code, globals=None)`

`code` 为 `str`，在调用这一刻按 2.2.1 的规则解析为若干条表达式。

`eval_isolated(code)` 等价于调用一个无形参、无捕获的匿名函数 `func(){code}`，
只是这次调用运行在一个全新、独立的全局作用域上（`_L` 即其 `_G`）：

调用处的局部/全局变量、global 标记集完全不可见，与调用处之间没有任何共享状态。
函数体的一般规则照常适用，不为 `eval_isolated` 单独定义。

`globals` 若传入，必须是 `dict`，直接作为这个新帧的 `_G` 本身。
