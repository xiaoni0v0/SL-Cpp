...
- int、decimal 字面量都可以带科学计数法后缀 `[eE][+-]?digits`
  ...
  - 结果为 int 时，指数不得超过 `65536`，超出则抛出 `SyntaxError`；
...
1. `init`, `cond`, `inc` 为表达式或为空，三者用 `;` 或至少一个换行无歧义地分割（若某个槽为空，则必须使用 `;`）；
...
复数字面量）、二元运算符 in、更细分的异常、raw_int、raw_float。
