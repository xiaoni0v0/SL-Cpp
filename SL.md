...
- 否则先解析第一个表达式：若其本身是 `**` 展开项或其后跟 `:` 则为字典字面量；
- 否则为复合表达式。
...
`==` 与 `!=` 的终局回退：上述两侧全部弃权后，这两个运算符不报错，而是由解释器按对象身份兜底，
`a == b` 取 `a is b`，`a != b` 取 `not (a is b)`。
`<`、`<=`、`>`、`>=` 没有这条兜底，两侧都弃权就抛 `TypeError`。
...
对 `global identifier` 求值，要求 `identifier` 是未出现的名称、局部变量、形参、值捕获之一，而不能是引用捕获，否则抛出 `NameError`。
...
抽象基类。定义数值的公共契约：四则运算与相等比较。

`int`、`decimal`、`complex`、`bool` 均为其子类。
