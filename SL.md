...
其中 `identifier` 为标识符。捕获列表内标识符不可重复，否则抛出 `SyntaxError`。
...
4. 对运算符：
    ...
    5. 对赋值运算符
        ...
        4. 复合赋值 `target op= expr`
           若 `target` 是标识符，按 3.10 所述的规则（含 3.10.4 捕获）先读取其当前值；若是属性/元素访问，按各自规则读取。
           再对 `expr` 求值，计算 `旧值 op expr`，写入结果到 `target`。
...
- `x op= expr`（其中 `op` 为支持的复合赋值运算的运算符），等价于 `x = x op expr`：
  `x` 的读、写均按 3.10 所述的作用域/捕获规则处理，读在 `expr` 求值之前，写在其后。
...
帧是该次函数调用或类体执行的运行环境，其中包含一个局部变量名到对象的局部字典、**global 标记集**、**存活标记**等。
记当前帧（栈顶）的局部字典为 `_L`、全局帧（栈底）的为 `_G`。
读/写/删一个变量即操作某帧局部字典中的对应项，全局作用域中 `_L` 即 `_G`。

存活标记初始为真；帧弹出（函数调用正常返回、异常传播退出、类体执行完毕）时置为假。全局帧永远存活。
...
2. 否则：
    - 读取：返回 `_L` 中该项的值；不存在则抛出 `NameError`。
...
1. **值捕获** `x`：函数建立时，按当时所在作用域的规则读取标识符 `x` 的值，将结果对象绑定为该函数的一个“隐藏默认值”。
   它不是真正的形参，不参与实参匹配，语义等价于 3.10.5 中用不参与传参的默认值形参捕获对象的写法。
   此后函数体内对 `x` 的读、写、删，与普通局部变量完全一致。

2. **引用捕获** `&x`：函数建立时，只记录 (定义帧, `x`) 这一对，不读取也不要求此刻已存在。
   此后函数体内对 `x` 的读、写、删，改为对**定义帧**按 3.10.3 的规则处理。
   若定义帧的存活标记已为假，则抛出 `NameError`。

被捕获的标识符（无论值/引用捕获）在函数体内不得再用 `global` 声明，否则抛出 `SyntaxError`。
...
mm = f()
mm()                          # f 已返回，mm 的定义帧存活标记为假，NameError
```
...
引用捕获可以让多个函数共享同一个变量本身（而不是共享同一个可变对象）：
