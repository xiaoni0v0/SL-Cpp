...
任意作用域内，只能访问和修改全局作用域和当前的局部作用域中的变量。无法访问和修改“较外侧的局部”中的变量。

#### 3.9.1 变量即字典项

解释器层面，全局作用域就是字典 `_G`，当前局部作用域就是字典 `_L`（二者均为可读可写的实时视图）。
对一个变量的**读取、写入、删除**，本质上都是对 `_L` 或 `_G` 中以变量名为键的项的读取、写入、删除。
在全局作用域中，`_L` 与 `_G` 指向同一个字典。

将“确定一个标识符 `identifier` 操作的是 `_L` 还是 `_G`”这一过程称为**作用域确定**。
读取、写入、删除共用同一套作用域确定规则。
...
1. 若 `identifier` 在 global 标记集中 → 确定为 `_G`；
2. 否则若 `_L` 中存在该项 → 确定为 `_L`；
3. 否则若 `_G` 中存在该项 → 确定为 `_G`；
4. 否则 → 未绑定。

在此基础上：

- 读取：返回所确定的字典中该项的值；若未绑定，抛出 `NameError`。

- 写入 `identifier = expr`：
    - 确定为 `_L` 或 `_G` → 更新该字典中的项；
    - 未绑定 → 在 `_L` 中新建该项。

  即：除非用 `global`，否则赋值永远不会在 `_G` 中新建变量。

- 删除 `del identifier`：从所确定的字典中删除该项；若未绑定，抛出 `NameError`。

#### 3.9.4 要点与惯用法

- 读取会自然穿透到全局，因此内置函数（`print` 等）和全局变量无需 `global` 即可读取。
- 裸赋值不污染全局：自由名字写入 `_L`；而对一个 `_G` 中已存在的同名变量，裸赋值会更新那个全局变量而非新建局部。
- 若要在局部 shadow 一个同名全局，请显式写 `_L['identifier'] = expr`（直接向局部帧写入，立即生效）。
- 若要在函数内创建或修改全局变量，使用 `global identifier`。
- 代价（动态确定的固有性质）：一个自由名字确定为 `_L` 还是 `_G`，取决于求值那一刻 `_G` 中是否已有同名项，因此同一函数在不同调用下可能不同。
  这是为换取 `_G`/`_L` 的实时动态能力而付出的代价。
...
func f() {
    print(g)          # 读：_L 无 → _G 有 → 1，无需 global
    g = g + 1         # _G 有 g → 读写都是全局 g，自洽；f 结束后全局 g == 2
    y = 5             # y 自由 → 新建局部，不污染全局
    _L['z'] = 9       # 显式建局部 z（可 shadow 同名全局）
    print(z)          # _L 有 z → 9
}
```

*注*：SL 不支持闭包。这是有意为之——闭包的隐式捕获会引入隐藏的依赖关系，降低代码可读性。需要维护状态时，推荐使用匿名类显式管理。

你可以使用参数默认值来模拟部分闭包行为。例如：
