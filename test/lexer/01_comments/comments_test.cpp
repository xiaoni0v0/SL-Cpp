...
TEST_SUITE("注释") {
    ...
    TEST_CASE("单行注释：本身不产生任何 token") { CHECK(lex_dump(U"# 整行都是注释") == ""); }
    ...
    TEST_CASE("多行注释：本身不产生任何 token") { CHECK(lex_dump(U"/* just a comment */") == ""); }
    ...
    TEST_CASE("多行注释：不支持嵌套，内层的 */ 直接结束整个注释（SL.md 明确举的例子）") {
