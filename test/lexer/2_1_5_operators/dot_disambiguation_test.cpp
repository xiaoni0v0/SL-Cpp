...
TEST_CASE("`....f()` 表示调用 Ellipsis 的 f 方法（4 个点 = 3 个点的 Ellipsis + 1 个点）") {
    CHECK(lex_dump(U"....f()") == "LITERAL_ELLIPSIS SIGN_DOT IDENTIFIER(f) SIGN_LPAREN SIGN_RPAREN");
}
...
TEST_CASE("两个数字中间夹三个点：不是小数点（数字侧只认单个点接数字），是数字 + Ellipsis + 数字") {
