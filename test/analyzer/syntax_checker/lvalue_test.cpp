...
TEST_CASE("复合赋值只允许简单左值（标识符/索引/属性），不允许解构") {
    ...
    CHECK_NOTHROW(check_program(U"a[0] += 1"));
    CHECK_NOTHROW(check_program(U"a.b += 1"));
    check_throws_with(U"(a, b) += x", "identifier, attribute access, or index expression expected before op=");
}

TEST_CASE("左值根节点内部的子表达式仍然要完整 check()——之前 check_lvalue 对 Index/Attr 直接"
    "返回、完全跳过了 object_/args_，导致里面的问题被忽略") {
    // a[break] = 1：break 在这里没有被任何循环包着，之前会被 check_lvalue 完全跳过检查，现在要报错
