...
TEST_CASE("按无穷位补码语义（SL.md 3.4.2 原例，本项目 int 字面量目前只支持十进制，255 即 0xff）") {
...
TEST_CASE("float 不参与位运算，不折") {
    CHECK(fold_json(U"1.0 & 1") == nlohmann::json{
          {"type", "OpBinary"}, {"op", "&"}, {"left", float_lit("1.0")}, {"right", int_lit("1")}
          });
}
