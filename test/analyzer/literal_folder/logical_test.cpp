...
TEST_CASE("or：左真则取左，左假则取右") {
    ...
    CHECK(fold_json(U"[] or [1]") == nlohmann::json{{"type", "LiteralList"}, {"items", {int_lit("1")}}});
}

TEST_CASE("and/or 不短路地各自折叠两边，被选中的一侧即使是复杂表达式也整体挪走") {
    // 右边 f() 不是字面量，但因为左边 True 为真，and 直接取右边（不需要右边本身可折）
    CHECK(fold_json(U"True and f()") == nlohmann::json{
    ...
    CHECK(fold_json(U"False or f()") == nlohmann::json{
          {"type", "Call"}, {"object", {{"type", "Identifier"}, {"identifier", "f"}}},
          {"args", nlohmann::json::array()}, {"kwargs", nlohmann::json::array()}
          });
}
...
TEST_CASE("左操作数含变量、真值未知时不折") {
    CHECK(fold_json(U"x and 1") == nlohmann::json{
