...
TEST_CASE("dict 不参与任何折叠，含 dict 操作数的比较也不折") {
    CHECK(fold_json(U"{1: 2} == {1: 2}") == nlohmann::json{
          {"type", "Compare"},
          {"operands", {
          nlohmann::json{
          {"type", "LiteralDict"}, {"items", {nlohmann::json{{"key", int_lit("1")}, {"val", int_lit("2")}}}}
          },
          nlohmann::json{
          {"type", "LiteralDict"}, {"items", {nlohmann::json{{"key", int_lit("1")}, {"val", int_lit("2")}}}}
          }
...
TEST_CASE("跨类型比较大小不可比，不折；但 == 恒成立（跨类型必不相等）") {
    CHECK(fold_json(U"1 < 'a'") == nlohmann::json{
...
TEST_CASE("链上含变量不折") {
    CHECK(fold_json(U"1 < x < 3") == nlohmann::json{
          {"type", "Compare"},
          {"operands", {int_lit("1"), {{"type", "Identifier"}, {"identifier", "x"}}, int_lit("3")}},
          {"ops", {"<", "<"}}
          });
}

TEST_CASE("is 一律不折（对象同一性没法在编译期安全预判，哪怕是 None）") {
    CHECK(fold_json(U"None is None") == nlohmann::json{
          {"type", "Is"}, {"operands", {none_lit(), none_lit()}}
          });
    CHECK(fold_json(U"1 is 1") == nlohmann::json{
          {"type", "Is"}, {"operands", {int_lit("1"), int_lit("1")}}
          });
}
