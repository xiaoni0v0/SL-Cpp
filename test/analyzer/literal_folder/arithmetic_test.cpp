...
TEST_CASE("除以 0 一律不折，交给运行时报 MathError") {
    CHECK(fold_json(U"1 / 0") == nlohmann::json{
    ...
    CHECK(fold_json(U"1.0 / 0.0") == nlohmann::json{
          {"type", "OpBinary"}, {"op", "/"}, {"left", float_lit("1.0")}, {"right", float_lit("0.0")}
          });
}
...
TEST_CASE("** 指数为负，结果是 float") {
    CHECK(fold_json(U"2 ** -1") == float_lit("0.5"));
}
...
TEST_CASE("~ 对 float 不折，交给运行时报错") {
    CHECK(fold_json(U"~1.5") == nlohmann::json{
          {"type", "OpUnary"}, {"op", "~"}, {"operand", float_lit("1.5")}
          });
}

TEST_CASE("含变量/调用的子表达式不折，只递归折内部能折的部分") {
    CHECK(fold_json(U"x + 1") == nlohmann::json{
          {"type", "OpBinary"}, {"op", "+"},
          {"left", {{"type", "Identifier"}, {"identifier", "x"}}}, {"right", int_lit("1")}
          });
    CHECK(fold_json(U"1 + 2 + x") == nlohmann::json{
          {"type", "OpBinary"}, {"op", "+"}, {"left", int_lit("3")},
