...
TEST_SUITE("ExprFolder Call 子表达式折叠") {
    ...
    TEST_CASE("object_ 会被折叠") {
        CHECK(fold_json(U"(1 + 1)()")["object"] == int_lit("2"));
    }
    ...
    TEST_CASE("*expr 展开的 operand_ 也会被折叠") {
        ...
        CHECK(
            ...
            nlohmann::json{
                {"type", "Star"},
                {"operand", {{"type", "LiteralTuple"}, {"items", {int_lit("2")}}}}
            }
        );
    }
    ...
    TEST_CASE("keyword_args_ 的 value_ 会被折叠（普通关键字实参、**展开都一样）") {
        ...
        CHECK(result["keyword_args"][0] == nlohmann::json{{"keyword", "x"}, {"value", int_lit("2")}});
        CHECK(
            result["keyword_args"][1]["value"] ==
            nlohmann::json{
                {"type", "DoubleStar"},
                {"operand",
                 {{"type", "LiteralTuple"}, {"items", {int_lit("2"), int_lit("4")}}}}
            }
        );
    }
...
TEST_SUITE("ExprFolder Index/Attr 子表达式折叠") {
    ...
    TEST_CASE("Index 的 args_ 里每一项都会被折叠") {
        CHECK(
            fold_json(U"a[1 + 1, 2 + 2]")["args"] == nlohmann::json::array({int_lit("2"), int_lit("4")})
        );
    }
...
TEST_SUITE("ExprFolder Assign/CompoundAssign 子表达式折叠") {
    ...
    TEST_CASE("Assign 的目标是 Index/Attr 时，target_ 里的子表达式（比如索引参数）也会被折叠") {
        CHECK(fold_json(U"a[1 + 1] = 1")["target"]["args"] == nlohmann::json::array({int_lit("2")}));
    }
