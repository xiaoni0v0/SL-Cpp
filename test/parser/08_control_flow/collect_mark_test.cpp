...
// for [mark] (i : xs) body 的期望形状，只有 collect 一项不同
...
TEST_SUITE("收集模式记号——非法组合") {
    ...
    TEST_CASE("两位不配套") {
        check_parse_throws_with(U"for $ ** (i : xs) body", "pairs with '*', not '**'");
    ...
    TEST_CASE("只有第二位，没有第一位") {
        check_parse_throws_with(U"for * (i : xs) body", "must follow a collect mark");
        check_parse_throws_with(U"for ** (i : xs) body", "must follow a collect mark");
