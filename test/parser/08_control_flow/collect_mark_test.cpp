...
// for [mark] (i in xs) body 的期望形状，只有 collect 一项不同
...
TEST_SUITE("收集模式记号——四种合法写法") {
    ...
    TEST_CASE("迭代模式上的四种记号") {
        CHECK(parse_json(U"for (i in xs) body") == for_iter("none"));
        CHECK(parse_json(U"for $ (i in xs) body") == for_iter("$"));
        CHECK(parse_json(U"for $ * (i in xs) body") == for_iter("$ *"));
        CHECK(parse_json(U"for $$ (i in xs) body") == for_iter("$$"));
        CHECK(parse_json(U"for $$ ** (i in xs) body") == for_iter("$$ **"));
    }
...
TEST_SUITE("收集模式记号——非法组合") {
    ...
    TEST_CASE("两位不配套") {
        check_parse_throws_with(U"for $ ** (i in xs) body", "pairs with '*', not '**'");
    ...
    TEST_CASE("只有第二位，没有第一位") {
        check_parse_throws_with(U"for * (i in xs) body", "must follow a collect mark");
        check_parse_throws_with(U"for ** (i in xs) body", "must follow a collect mark");
    ...
    TEST_CASE("$$ 必须连写，$ $ 不是 $$") {
        ...
        CHECK_THROWS_AS(parse_as_file(U"for $ $ (i in xs) body"), SyntaxError);
        CHECK_THROWS_AS(parse_as_file(U"for $ $ ** (i in xs) body"), SyntaxError);
    }
