// 跨分组：AST 节点里 Position 相关字段是否记录正确。
// AstNode::to_json() 不序列化位置信息（纯结构性 dump），所以这里全部直接 dynamic_cast 取字段断言，
// 不走 parse_json()。列号从 1 开始（跟 lexer 的 test/lexer/common/position_tracking_test.cpp
// 一致）。
...
TEST_SUITE("位置追踪——AstNode::pos_ 基本行为") {
    ...
    TEST_CASE("跨行合并的表达式：pos_ 是合并前第一个 token 的位置") {
        ...
        CHECK(add->op_pos_.row == 1);
        CHECK(add->op_pos_.col == 7); // '+': e(5) (6)+(7)
    }
}

TEST_SUITE("位置追踪——运算符自己的位置") {

    TEST_CASE("二元运算符 op_pos_ 跟整个节点的 pos_（左操作数起始位置）不同") {
        const AstNodePtr node{parse_single(U"a + b")};
        const auto *bin{dynamic_cast<AstNodeOpBinary *>(node.get())};
        REQUIRE(bin != nullptr);
        CHECK(bin->pos_.col == 1);    // 'a'
        CHECK(bin->op_pos_.col == 3); // '+': a(1) (2)+(3)
    }

    TEST_CASE("前缀一元运算符：pos_ 和 op_pos_ 重合（运算符本身就是起点）") {
        const AstNodePtr node{parse_single(U"-x")};
        const auto *un{dynamic_cast<AstNodeOpUnary *>(node.get())};
        REQUIRE(un != nullptr);
        CHECK(un->pos_.col == 1);
        CHECK(un->op_pos_.col == 1);
    }

    TEST_CASE("后缀 ?/!：pos_ 是操作数起始位置，op_pos_ 是问号/感叹号自己的位置") {
        const AstNodePtr node{parse_single(U"x?")};
        const auto *un{dynamic_cast<AstNodeOpUnary *>(node.get())};
        REQUIRE(un != nullptr);
        CHECK(un->pos_.col == 1);    // 'x'
        CHECK(un->op_pos_.col == 2); // '?'
    }

    TEST_CASE("复合赋值 op_pos_ 是 op= 自己的位置") {
        const AstNodePtr node{parse_single(U"x += 1")};
        const auto *ca{dynamic_cast<AstNodeCompoundAssign *>(node.get())};
        REQUIRE(ca != nullptr);
        CHECK(ca->pos_.col == 1);    // 'x'
        CHECK(ca->op_pos_.col == 3); // '+=': x(1) (2)+(3)=(4)
    }

    TEST_CASE("属性访问 dot_pos_ 是 '.' 自己的位置") {
        ...
        CHECK(attr->dot_pos_.col == 2); // '.'
    }

    TEST_CASE("调用 args_.paren_pos_ 是 '(' 自己的位置") {
        const AstNodePtr node{parse_single(U"f(1)")};
        const auto *call{dynamic_cast<AstNodeCall *>(node.get())};
        REQUIRE(call != nullptr);
        CHECK(call->pos_.col == 1);             // 'f'
        CHECK(call->args_.paren_pos_.col == 2); // '('
    }

    TEST_CASE("import 调用形态 args_.paren_pos_ 是 '(' 自己的位置，pos_ 是 'import' 的位置") {
        ...
        CHECK(call->args_.paren_pos_.col == 7); // '('
    }
    ...
    TEST_CASE("索引 bracket_pos_ 是 '[' 自己的位置") {
        ...
        CHECK(index->bracket_pos_.col == 2); // '['
    }
    ...
    TEST_CASE("链式调用/索引/属性访问，每一层各自的运算符位置都正确") {
        ...
        const auto *index{dynamic_cast<AstNodeIndex *>(node.get())};
        REQUIRE(index != nullptr);
        CHECK(index->bracket_pos_.col == 9); // '['
        const auto *attr{dynamic_cast<AstNodeAttr *>(index->object_.get())};
        REQUIRE(attr != nullptr);
        CHECK(attr->dot_pos_.col == 4); // '.'
    }
}

TEST_SUITE("位置追踪——链式比较 / is 链每个运算符自己的位置") {

    TEST_CASE("链式比较：op_positions_ 跟 ops_ 一一对应，各自指向自己的运算符") {
        ...
        REQUIRE(cmp != nullptr);
        REQUIRE(cmp->op_positions_.size() == 2);
        CHECK(cmp->op_positions_[0].col == 3); // 第一个 '<'
        CHECK(cmp->op_positions_[1].col == 7); // '<='
    }

    TEST_CASE("is 链：op_positions_ 指向每个 'is' 自己的位置") {
        ...
        REQUIRE(is_node->op_positions_.size() == 2);
        CHECK(is_node->op_positions_[0].col == 3); // 第一个 'is'
        CHECK(is_node->op_positions_[1].col == 8); // 第二个 'is'
    }
}

TEST_SUITE("位置追踪——装饰器链每个 '@' 自己的位置") {

    TEST_CASE("紧邻 func 的装饰器链：decorator_positions_ 跟 decorators_ 一一对应") {
        ...
        REQUIRE(func_node->decorator_positions_.size() == 2);
        CHECK(func_node->decorator_positions_[0].col == 1); // 第一个 '@'
        CHECK(func_node->decorator_positions_[1].col == 5); // 第二个 '@'
    ...
    TEST_CASE("紧邻 class 的装饰器链同理") {
        ...
        REQUIRE(cls != nullptr);
        REQUIRE(cls->decorator_positions_.size() == 2);
        CHECK(cls->decorator_positions_[0].col == 1);
        CHECK(cls->decorator_positions_[1].col == 5);
        CHECK(cls->pos_.col == 1);
    }

    TEST_CASE("没有装饰器时，decorator_positions_ 为空，节点自身位置从关键字算起") {
        const AstNodePtr node{parse_single(U"func f() {}")};
        const auto *func_node{dynamic_cast<AstNodeFunc *>(node.get())};
        REQUIRE(func_node != nullptr);
        CHECK(func_node->decorator_positions_.empty());
