...
TEST_SUITE("2.1.5 优先级——四则/位运算/范围") {
    ...
    TEST_CASE("* 比 + 优先级高：1 + 2 * 3 == 1 + (2 * 3)") {
        CHECK(
            parse_json(U"1 + 2 * 3") == nlohmann::json{{"type", "OpBinary"},
                                                       {"op", "+"},
                                                       {"left", int_lit("1")},
                                                       {"right",
                                                        {{"type", "OpBinary"},
    ...
    TEST_CASE("* / // % 同级左结合：10 // 3 % 2 == (10 // 3) % 2") {
        CHECK(
            parse_json(U"10 // 3 % 2") == nlohmann::json{{"type", "OpBinary"},
                                                         {"op", "%"},
                                                         {"left",
                                                          {{"type", "OpBinary"},
                                                           {"op", "//"},
    ...
    TEST_CASE("** 右结合：2 ** 3 ** 2 == 2 ** (3 ** 2)") {
        CHECK(
            parse_json(U"2 ** 3 ** 2") == nlohmann::json{{"type", "OpBinary"},
                                                         {"op", "**"},
                                                         {"left", int_lit("2")},
                                                         {"right",
                                                          {{"type", "OpBinary"},
    ...
    TEST_CASE("** 比一元负号优先级高：-2 ** 2 == -(2 ** 2)，不是 (-2) ** 2") {
        CHECK(
            parse_json(U"-2 ** 2") == nlohmann::json{{"type", "OpUnary"},
                                                     {"op", "-"},
                                                     {"operand",
                                                      {{"type", "OpBinary"},
                                                       {"op", "**"},
    ...
    TEST_CASE("位运算优先级：& > ^ > |，1 | 2 ^ 3 & 4 == 1 | (2 ^ (3 & 4))") {
        CHECK(
            parse_json(U"1 | 2 ^ 3 & 4") == nlohmann::json{{"type", "OpBinary"},
                                                           {"op", "|"},
                                                           {"left", int_lit("1")},
                                                           {"right",
                                                            {{"type", "OpBinary"},
