...
TEST_CASE("多个装饰器按书写顺序进 decorators_") {
    CHECK(parse_json(U"@dec1 @dec2 func f() {}")["decorators"] == nlohmann::json::array({
        ident("dec1"), ident("dec2")
        }));
}
...
TEST_CASE("装饰器表达式本身可以是调用") {
    CHECK(parse_json(U"@dec(1, 2) func f() {}")["decorators"] == nlohmann::json::array({
        nlohmann::json{
        {"type", "Call"}, {"object", ident("dec")},
        {
...
TEST_CASE("单个装饰器包裹普通表达式") {
    CHECK(parse_json(U"@dec x") == nlohmann::json{
          {"type", "Decorator"}, {"decorator", ident("dec")}, {"target", ident("x")}
          });
}

TEST_CASE("多个装饰器：从最贴近 expr 的开始向外包裹，@d1 @d2 x == d1(d2(x))") {
    CHECK(parse_json(U"@d1 @d2 x") == nlohmann::json{
          {"type", "Decorator"}, {"decorator", ident("d1")},
          {"target", {{"type", "Decorator"}, {"decorator", ident("d2")}, {"target", ident("x")}}}
          });
}

TEST_CASE("装饰器目标是赋值表达式") {
    CHECK(parse_json(U"@dec x = 1") == nlohmann::json{
          {"type", "Decorator"}, {"decorator", ident("dec")},
          {
          "target",
          {
          {"type", "Assign"}, {"target", ident("x")},
...
TEST_CASE("装饰器后面缺表达式报错") {
    CHECK_THROWS_AS(parse_program(U"@dec"), SyntaxError);
}
