...
nlohmann::json param(const char *id, const nlohmann::json &type_annotation, const nlohmann::json &default_value) {
...
nlohmann::json all_params(const nlohmann::json &positional = nlohmann::json::array(),
    ...
    return nlohmann::json{
        {"positional", positional}, {"var_args", var_args}, {"kw_only", kw_only}, {"var_kwargs", var_kwargs}
    };
}
} // namespace
...
TEST_CASE("最简单的具名函数") {
    CHECK(parse_json(U"func f() {}") == nlohmann::json{
          ...
          {"captures", nlohmann::json::array()}, {"params", all_params()},
...
TEST_CASE("普通形参") {
    CHECK(parse_json(U"func f(a, b) {}")["params"]["positional"] == nlohmann::json::array({
        param("a", nullptr, nullptr), param("b", nullptr, nullptr)
        }));
}
...
TEST_CASE("类型注解") {
    CHECK(parse_json(U"func f(a: int) {}")["params"]["positional"] == nlohmann::json::array({
        param("a", ident("int"), nullptr)
        }));
}

TEST_CASE("默认值") {
    CHECK(parse_json(U"func f(a = 1) {}")["params"]["positional"] == nlohmann::json::array({
        param("a", nullptr, int_lit("1"))
        }));
}

TEST_CASE("类型注解 + 默认值同时出现") {
    CHECK(parse_json(U"func f(a: int = 1) {}")["params"]["positional"] == nlohmann::json::array({
        param("a", ident("int"), int_lit("1"))
        }));
}

TEST_CASE("*args / **kwargs：各自是独立的 var_args/var_kwargs 子段，不再混进 positional") {
    CHECK(parse_json(U"func f(*args, **kwargs) {}")["params"]
...
TEST_CASE("至多一个 **kwargs") {
    check_parse_throws_with(U"func f(**x, **y) {}", "at most one **kwargs parameter is allowed");
}
...
TEST_CASE("**kwargs 必须是形参列表最后一项，后面不能再有任何形参") {
    ...
    check_parse_throws_with(U"func f(**kw, **kw2) {}", "at most one **kwargs parameter is allowed");
}
