...
//   见 SL.md 展开语法）或其后紧跟 ':' 为字典字面量；都不满足为复合表达式。
// 这里只测判别规则本身和复合表达式的形状；字典各类项的具体语义（key 是否任意表达式等）已在
// 01_literals/literals_test.cpp 测过。
...
TEST_SUITE("字典展开项：按表达式本身是不是 ** 展开判定（不是靠有没有冒号反推）") {
...
TEST_SUITE("{} 内部是独立的语句语境：块内换行不受外层括号续行规则影响") {

    // SL.md 的表达式分隔规则按"块"递归适用：括号内的换行视作续行（表达式不完整则合并），
