...
TEST_CASE("只有分隔符、没有任何表达式的 {} 仍是空复合表达式") {
    const auto empty_compound = nlohmann::json{{"type", "Compound"}, {"exprs", nlohmann::json::array()}};
...
TEST_CASE("尾随分隔符：{a;} / {a\\n} 都是单语句复合表达式") {
    const auto single = nlohmann::json{{"type", "Compound"}, {"exprs", nlohmann::json::array({ident("a")})}};
...
TEST_CASE("嵌套复合表达式：外层在调用实参里，内层还有自己的换行，逐层语境正确切换") {
    CHECK(parse_json(U"f({a\n{b\nc}})")["args"][0] == nlohmann::json{
          {"type", "Compound"}, {
          "exprs", nlohmann::json::array({
              ident("a"),
              {{"type", "Compound"}, {"exprs", nlohmann::json::array({ident("b"), ident("c")})}}
              })
          }
          });
}
...
TEST_CASE("调用实参里的多行字典照常工作（字典跨行靠自身对换行的显式容忍，与括号续行无关）") {
    CHECK(parse_json(U"f({\nk: v,\nk2: v2,\n})")["args"][0] == nlohmann::json{
          {"type", "LiteralDict"},
          {
          "items", nlohmann::json::array({
              {{"key", ident("k")}, {"val", ident("v")}},
...
TEST_CASE("调用实参里的字典：键与 ':' 之间的换行照常允许") {
    CHECK(parse_json(U"f({k\n: v})")["args"][0] == nlohmann::json{
          {"type", "LiteralDict"},
          {"items", nlohmann::json::array({{{"key", ident("k")}, {"val", ident("v")}}})}
          });
}
...
TEST_CASE("字典值是复合表达式、整体又在调用实参里：逐层语境正确切换") {
    CHECK(parse_json(U"f({k: {a\nb}})")["args"][0] == nlohmann::json{
          ...
          "items", nlohmann::json::array({
              {
              {"key", ident("k")},
              {"val", {{"type", "Compound"}, {"exprs", nlohmann::json::array({ident("a"), ident("b")})}}}
              }
              })
          }
          });
...
TEST_CASE("'}' 之后回到外层语境：括号内 '}' 后面的换行仍按括号续行规则合并") {
    CHECK(parse_json(U"f({a; b}\n.c)")["args"][0] == nlohmann::json{
          {"type", "Attr"},
          {"object", {{"type", "Compound"}, {"exprs", nlohmann::json::array({ident("a"), ident("b")})}}},
          {"attr", "c"}
          });
}
