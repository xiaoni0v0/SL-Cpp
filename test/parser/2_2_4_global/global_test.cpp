...
// 注意：identifier 本身就是标识符 token，不是"解析成表达式再校验形状"，parser 直接
// expect(IDENTIFIER)。
...
TEST_SUITE("2.2.4 global") {
    ...
    TEST_CASE(
        "global 只消耗一个标识符，后面多余的内容如果不能接成后缀运算，会在外层因为缺分隔符报错"
    ) {
    ...
    TEST_CASE("缺少标识符时报错") { CHECK_THROWS_AS(parse_program(U"global"), SyntaxError); }
