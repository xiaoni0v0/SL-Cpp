...
// decimal 的超越函数（ln / log10 / exp / **）用的整数层算法，逐个对应 Python `_pydecimal` 里
// 那一组 `_xxx` 辅助函数。这一层只跟整数打交道，不认识 BigDec、也不认识上下文和信号：
// 它算出的是"带 p 位精度、误差不超过 1 ulp 的近似值"，"要不要再多算几位才能定下舍入方向"
// 由上层（BigDec）循环判断。
//
// 这些算法用定点整数模拟实数：一个实数 z 用整数 round(z * M) 表示，M 通常取 10^p 或 2^k。
// 误差界都是 Python 那边论证过的，改动这里的任何一个常数（尤其是 kTaylorL）都会破坏上层
// "算到能定夺为止"的循环所依赖的前提。
namespace dec_math {

// 泰勒级数展开前的参数缩减深度，Python 里 _ilog/_iexp 的默认参数 L
...
// 最接近 sqrt(n) 的整数，a 是初始近似（随便一个正整数都行，越接近收敛越快）。
// 调用方保证 n > 0、a > 0
...
// M * exp(x/M) 的整数近似。调用方保证 M > 0 且 x/M 的绝对值不大（0 <= x/M <= 2.4 时误差 < 60）
[[nodiscard]] BigInt iexp(const BigInt &x, const BigInt &m);

// 把 exp(c*10^e) 近似成 d*10^f，返回 (d, f)：10^(p-1) <= d <= 10^p，且 d 的误差不超过 1
...
// 且 c 的误差不超过 1。调用方保证底数为正且不等于 1、指数不为 0
...
// 特意绕开"先把 10^|e| 造出来"，e 是个很大的负数时那一步会白白炸开
